...
export default function ProtectedWhatsAppCampaign({ prefill, onPrefillConsumed }: ProtectedWhatsAppCampaignProps = {}) {
  ...
  useEffect(() => {
    ...
    const auth = sessionStorage.getItem('whatsappCampaignAuth');
    if (auth === 'true') {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    // Check password (same as email campaign for consistency)
    if (password === 'flashfire@2025') {
      setIsAuthenticated(true);
      sessionStorage.setItem('whatsappCampaignAuth', 'true');
      setError('');
    } else {
